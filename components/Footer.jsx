  ...
  useEffect(() => {
    const getData = async () => {
      const data = await fetch(
        "https://api.github.com/repos/tannerhallman/tannerhallman.com-v2"
      ).then((res) => res.json());
      setMetaData({
  ...
  return (
    <div className=' flex select-none text-sm  py-16 mt-16 flex-col h-max items-center mx-auto justify-center'>
      <div className='flex justify-center space-x-4 md:space-x-6 dark:text-white/70 text-gray-500 text-[0.6rem] sm:text-xs md:text-sm lg:text-md mt-2'>
        ...
        <Link
          href='https://github.com/tannerhallman?tab=repositories'
          target='blank'
          className=' dark:hover:text-blue-400 hover:text-blue-600 font-semibold'>
          {" "}
          {/* GitHub{" "} */}
      ...
      <a
        ...
        <div className='flex space-x-4 '>
          <div className='flex dark:group-hover:text-blue-400 group-hover:text-blue-600  space-x-1 items-center '>
            <FaRegStar className='h-4 w-4 group-hover:stroke-[2.2px] dark:group-hover:fill-blue-400  group-hover:fill-purple-800 ' />
            <p className='font-semibold text-sm group-hover:font-extrabold '>
              {metaData.star}
            </p>
          </div>
          <div className='flex  dark:group-hover:text-blue-400 group-hover:text-blue-600 space-x-1 items-center '>
            <FiGitBranch className='h-4 w-4 group-hover:stroke-[2.2px]  dark:group-hover:stroke-blue-400 group-hover:stroke-purple-800 ' />
            <p className='font-semibold text-sm group-hover:font-extrabold '>
              {metaData.forks}
            </p>
          </div>
